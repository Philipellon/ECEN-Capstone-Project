...
#define OBSTACLE_THRESHOLD 10    // Obstacle detection threshold in cm
...
void echo_sensor(void *pvParameter) {
    ...
    while (1) {
        for (int i = 0; i < NUM_SENSORS; i++) {
            ...
            vTaskDelay(pdMS_TO_TICKS(100)); // Delay between each sensor read
        }
        ...
        */
        vTaskDelay(pdMS_TO_TICKS(100)); // Delay between each full sensor cycle
    }
}
